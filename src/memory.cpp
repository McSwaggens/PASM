...
void Memory::PrintDebugInformation ()
{
	std::cout << "----------------------------------------------" << std::endl;
	std::cout << "Free Blocks: " << std::endl;
	for (int i = 0; i < freeBlocks.size(); i++)
	{
		MemoryBlock* memoryBlock = freeBlocks[i];
		PrintBlockInformation (memoryBlock);
	}
	
	std::cout << "\nUsed Blocks: " << std::endl;
	for (int i = 0; i < usedBlocks.size(); i++)
	{
		MemoryBlock* memoryBlock = usedBlocks[i];
		PrintBlockInformation (memoryBlock);
	}
	std::cout << "----------------------------------------------" << std::endl;
}

void Memory::PrintBlockInformation (MemoryBlock* memoryBlock)
{
	std::cout << memoryBlock->address << "\n\t";
